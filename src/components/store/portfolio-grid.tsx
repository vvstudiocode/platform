    ...
    return (
        ...
        >
            <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-2 gap-4">
                    {items.map((item: PortfolioItem, index: number) => (
                        <a
                            ...
                            href={item.link || '#'}
                            className={cn(
                                "relative rounded-2xl overflow-hidden group w-full block transition-all hover:shadow-xl",
                        ...
                        >
                            ...
                            <div className="absolute inset-0 flex flex-col justify-end p-4 md:p-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <span className="text-[10px] md:text-xs text-white/70 font-medium uppercase mb-1">{item.category}</span>
                                <h3 className="text-white text-sm md:text-lg font-bold mb-3 line-clamp-2">{item.title}</h3>
                                {item.buttonText && (
                                    <div className="self-start px-4 py-1.5 bg-white text-black text-[10px] md:text-xs font-bold rounded-full transform translate-y-2 group-hover:translate-y-0 transition-transform">
                                        {item.buttonText}
                                    </div>
                                )}
                            </div>

                            <span className="absolute bottom-4 right-4 md:bottom-6 md:right-6 text-[10px] md:text-xs text-white font-medium tracking-wide uppercase group-hover:opacity-0 transition-opacity">
                                {item.title}
                            </span>
                        </a>
                    ))}
                    ...
                    {showViewAll && (
                        <a
                            href={viewAllUrl}
                            className="relative aspect-square md:aspect-[3/4] rounded-2xl overflow-hidden bg-gray-100 dark:bg-gray-800 flex items-center justify-center p-6 text-center border-2 border-dashed border-gray-300 dark:border-gray-700 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700 transition-all hover:shadow-md"
                        >
                            <div className="flex flex-col items-center">
                                <Plus className="w-8 h-8 md:w-10 md:h-10 mb-2 opacity-30" />
                                <p className="text-[10px] md:text-xs font-semibold opacity-50 uppercase">{viewAllText}</p>
                            </div>
                        </a>
                    )}
                </div>
                ...
                <a
                    href={viewAllUrl}
                    className="block w-full text-center mt-10 py-4 border border-gray-900 dark:border-white rounded-xl font-semibold hover:bg-gray-900 hover:text-white dark:hover:bg-white dark:hover:text-gray-900 transition-all"
                >
                    {viewAllText}
                </a>
            </section>
