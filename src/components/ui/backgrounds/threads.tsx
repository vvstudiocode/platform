...
/* 
  Threads Background
  Source: https://reactbits.dev/backgrounds/threads
  Adapted for Next.js/React by AI
*/
...
const Threads = ({
    ...
    useEffect(() => {
        ...
        // We add listeners, but we check the prop in the update loop or closure if valid
        // Actually, for cleaner code, let's keep listeners always active but maybe gate logic.
        // Or re-bind if interactive changes? 
        // For simplicity and performance, binding once is fine, overhead is minimal.
        container.addEventListener('mousemove', handleMouseMove);
        container.addEventListener('mouseleave', handleMouseLeave);

        function update(t: number) {
            if (programRef.current) {
                // We need to access the LATEST enableMouseInteraction. 
                // Since this closure is created once, we might need a ref for the prop.
                // However, let's just use the uniform update effect to handle 'disable'.
                // If we want to stop mouse tracking, we can just stop updating targetMouse.

                // For now, let's smoothing always run to return to center if disabled?
                // Or better: use a ref to track current mouse enabled state if we want to toggle inside the loop.
    ...
    // Update Uniforms Effect (Run when props change)
    useEffect(() => {
        if (programRef.current) {
            ...
            if (!enableMouseInteraction && programRef.current) {
