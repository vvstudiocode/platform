...
import { Plus, Package, Search, MoreVertical, Edit, Trash2 } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { ProductDeleteButton } from './product-delete-button'

// 取得總部商店 ID
async function getHQStoreId(supabase: any, userId: string) {
    // 嘗試找 slug = 'hq' 的商店，或者找第一個由當前用戶管理的商店
                            ...
                            products.map((product) => (
                                <tr key={product.id} className="border-b border-zinc-800 hover:bg-zinc-800/50">
                                    ...
                                    <td className="px-6 py-4">
                                        ...
                                        {product.cost > 0 && (
                                            <p className="text-xs text-zinc-500">成本: ${Number(product.cost).toLocaleString()}</p>
                                        )}
                                    </td>
                                    <td className="px-6 py-4">
